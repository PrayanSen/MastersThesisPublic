...
def main():
    ...
    # do finetuning if do_finetuning is True
    if ft_args.do_finetuning:
        ...
        model.add_classification_head(la_args.language_adapter, num_labels=glue_tasks_num_labels[glue_data_args.task_name])
        print("...done")

        # Set language adapter and head to active
        print("Set adapter plus head to active...")
        model.set_active_adapters(la_args.language_adapter)
        print("...done")

        # Unfreeze all weights if it is not the case yet
        print("Unfreeze all layers to train the whole model...")
    ...
    # if do_finetuning is set to False, load already finetuned model
    else:
        print("\nFinetuning is skipped because already trained model exists and will be loaded in next step.")
